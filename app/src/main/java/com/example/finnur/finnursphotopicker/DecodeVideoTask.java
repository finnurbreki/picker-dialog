...
class DecodeVideoTask extends AsyncTask<Pair<Bitmap, String>> {
     ...
     *             video.
     */
    public DecodeVideoTask(VideoDecodingCallback callback, ContentResolver contentResolver, Uri uri, int size) {
        mCallback = callback;
    ...
    protected Pair<Bitmap, String> doInBackground() {
        ...
        try {
            Bitmap bitmap = BitmapUtils.decodeVideoFromFileDescriptor(mRetriever, mContentResolver.openAssetFileDescriptor(mUri, "r").getFileDescriptor(), mSize);
